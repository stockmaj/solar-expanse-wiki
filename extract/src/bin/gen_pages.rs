...
struct Sirenix {
    ...
    /// Drives the dedicated Achievements page.  See `parse_sirenix.rs` /
    /// `Achievement` for the source-of-truth shape.
    #[serde(default)]
...
struct AchievementStat {
    /// Steam achievement id, e.g. `id_achievement_NotToday`.
    id: String,
    /// Player-facing name from locale.  Empty in the current dump (no
    /// locale entries exist for achievements); `page_achievements`
    /// falls back to a humanized form of `id`.
    #[serde(default)]
    name: String,
    /// `contract` / `spacecraft` / `launch_vehicle`.
    source_type: String,
    /// Source id — `contract_*`, `spacecraft_*`, `lv_*` / `id_Rocket_*`.
    source_id: String,
    /// Free-form description if available; empty in the current dump.
    #[serde(default)]
    description: String,
}
...
mod tests {
    ...
    // ---------- Achievements page ----------
    //
    // The achievements page is sourced from `Sirenix.achievements`, with three
    // sections (one per source_type).  Locale.json has no achievement keys, so
    // the player-facing name is derived from the id with a humanize helper.
